...
pos = [1, 2]
ax[0].bar(pos, np.mean(data_pc[:2], axis=1), color='cornflowerblue')
ax[1].bar(pos, np.mean(data_pc[2:], axis=1), color='cornflowerblue')
np.random.seed(2020)
for n in range(data_pc.shape[1]):
    ind = np.random.uniform(-0.1,0.1,1)
    ax[0].plot(pos + ind, data_pc[:2, n], 'o-', color='darkgray', ms=2, linewidth=0.2)
    ax[1].plot(pos + ind, data_pc[2:, n], 'o-', color='darkgray', ms=2, linewidth=0.2)

ax[2].bar(pos, np.mean(data_z[:2], axis=1), color='cornflowerblue')
ax[3].bar(pos, np.mean(data_z[2:], axis=1), color='cornflowerblue')
for n in range(data_z.shape[1]):
    ind = np.random.uniform(-0.1,0.1,1)
    ax[2].plot(pos + ind, data_z[:2, n], 'o-', color='darkgray', ms=2, linewidth=0.2)
    ax[3].plot(pos + ind, data_z[2:, n], 'o-', color='darkgray', ms=2, linewidth=0.2)

ax[0].set_ylim([-0.06, 0.11])
ax[1].set_ylim([-0.06, 0.11])
