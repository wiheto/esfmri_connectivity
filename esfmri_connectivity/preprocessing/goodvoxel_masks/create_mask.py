...
import pandas as pd
import os
parc = nib.load('./esfmri_connectivity/parcellation/tpl-MNI152NLin2009cAsym_res-02_atlas-frankenstein_dseg.nii.gz')
parcdata = parc.get_data()

parcinfo = pd.read_csv('./esfmri_connectivity/parcellation/tpl-MNI152NLin2009cAsym_res-01_atlas-frankenstein_dseg.tsv', sep='\t', index_col=[0])
...
for f in files: 
    ...
    for parcel in np.unique(parcdata[ind]):
        ...
        if (inmask/intemplate) < 0.5:
            ...
            parcinmask.append(1)
    maskinfo = pd.DataFrame(data={'name': parcinfo['name'], 'inmask': parcinmask})
    print(str(n) + ' parcels deleted')
    report.append(n)
    # Save output
    maskimg = nib.Nifti1Image(maskparc, parc.affine)
    nib.save(maskimg, rel_dir + 'masks/' + f.split('_voxdist')[0] + '_tpl-MNI152NLin2009cAsym_res-02_atlas-frankenstein_dseg.nii.gz')
    maskinfo.to_csv(rel_dir + 'masks/' + f.split('_voxdist')[0] + '_tpl-MNI152NLin2009cAsym_res-02_atlas-frankenstein_dseg.tsv', sep='\t')
    s, t, _ = f.split('_') 
    sub.append(s)
    task.append(t)

report = pd.DataFrame(data={'sub': sub, 'task': task, 'parcels_deleted': report})
